...
const handleSuccessfulPayment = async (req, res) => {
    try {
        ...
        const cart = await Cart.findById(paymentIntent.metadata.cartId).populate('items.item');
...
const handleWebhook = async (req, res) => {
    ...
    switch (event.type) {
        case 'checkout.session.completed':
            ...
            const cart = await Cart.findOne({ _id: session.metadata.cartId }).populate('items.item');
